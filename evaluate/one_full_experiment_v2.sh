...
python3 evaluate/perform_wsd.py -m $model_path -v $vocab_path -c $wsd_df_in -l $settings_path -s $embeddings_path -o $wsd_df_out -r $results -g $eval_setting -f $mfs_fallback -t $path_case_freq -a $use_case_strategy -p $path_plural_freq -b $use_number_strategy -y $path_lp -z $use_lp
